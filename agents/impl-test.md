...
3. **Code Result** — full most recent `impl-code` response
4. **Repair Context** — on `test-repair`: `### Route Context` block from `impl-verify`. On `test-sync`: `None.` in fresh mode, or repair evidence in fix mode.
5. **Fix Mode** — `yes` enables new tests for repair-target behaviors lacking stable coverage; `no` for fresh mode.
...
- `test-sync`: repair mechanical mismatches only (imports, renamed symbols, updated signatures). **Do not delete tests in this mode**, even if they reference symbols absent from the post-CODE inventory: a refactor that removed a public symbol may have left orphaned coverage. Such tests will fail to load and be flagged in Step 7 as `HARNESS_NOISY`; `impl-verify`'s `TEST_REPAIR` route handles them downstream.
...
**FAIL (spec ambiguous — cannot encode as deterministic tests):** `Testability` → `TASK_AUTHORED_TESTS`; `Tests Written`, `Files Modified`, `Files Created`, `Stable Evidence`, `Unsafe Evidence` → `None.`; `Evidence Classification` → `N/A`. State the ambiguity precisely in `### Summary` so the caller can escalate it.
