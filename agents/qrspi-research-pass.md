...
Read all `<artifact-root>/q-NN.md` files. Dispatch `qrspi-research-synthesizer`:
...
- If `### Synthesis Issues` is not `None.` or any `q-NN.md` changed, re-dispatch `qrspi-research-synthesizer` with the updated findings and the latest review output, then overwrite `<artifact-root>/summary.md`.
