...
- Check pre-phase completion markers:
  ...
  - Skeleton+Structure (Stage 4.5) complete if `structure.md` exists, or the route is quick-fix.
    - **Internal checkpoint:** if `skeleton-results.md` exists and its first line is `### Status — PASS` but `structure.md` is absent, the skeleton build and squash-merge completed but structure mapping did not. Set `next_stage: skeleton`; the skeleton orchestrator's Step 0 detects this state and self-resumes at the mapping sub-step (Step G) without re-running the build or re-merging code.
    - If `skeleton-results.md` exists but parses as `### Status — FAIL`, treat Stage 4.5 as incomplete with an explicit failure — surface via Error Handling before advancing to Plan.
