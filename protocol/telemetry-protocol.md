...
| Skeleton+Structure (4.5)| `slice`, `files_created`, `files_modified`, `squash_merged`, `structure_review_rounds`, `structure_terminal_state` (`clean` \| `unclean-cap`), `backward_loop` (when applicable)                                    |
...
**`gate_round_details` object array** (human-gated stages, optional):
...
[
  {
    ...
    "presented_at": "<YYYY-MM-DDTHH:MM:SSZ>",
...
- `unclean-cap` — reached the maximum round cap with outstanding concerns. For Goals and Design, the state feeds the stage-local human gate. For the Structure sub-step of Stage 4.5, `unclean-cap` proceeds without a human gate — the Plan reviewer and feasibility checker are the downstream safety net. For Plan/Replan, deepwork raises an unclean-cap question gate after observing this state.
