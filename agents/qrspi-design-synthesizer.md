---
description: Synthesizes a design document from goals, preserved requirements, research, and interactive design discussion. Structures the chosen approach, conceptual architectural patterns, vertical slices, slice dependency DAG, phases, replan gates, and test strategy. Read-only — never modifies project files.
mode: subagent
hidden: true
temperature: 0.1
steps: 10
...
- [ ] The design is concrete enough for `qrspi-skeleton` to select the thinnest implementable slice and for `qrspi-structure-mapper` to identify the relevant slice scope.
