...
1. **Merged-stage ownership.** You are the only deeplooper-dispatched stage between Goals and Design. `dl-questions` now runs only as your child.
...
Read every `research/iterations/round-*/q-NN.md` artifact written so far. Dispatch `dl-research-synthesizer`:
...
- Dispatch `dl-questions` for the next round:
